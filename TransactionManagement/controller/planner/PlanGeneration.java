...
import reconfigurationPlan.GettingStatusStep;
import reconfigurationPlan.PassivationAction;
import reconfigurationPlan.PassivationStep;
...
public class PlanGeneration {
	...
	void generatePlan() {
	    ...
	    for (String str : getFrontierComponent()) {
	    ...
	    PassivationStep passivationStep = reconfigurationPlanFactory.createPassivationStep();
	    plan.setPassivation(passivationStep);
	    for (String str : getRemovedComponents()) {
	    	PassivationAction passivationAction = reconfigurationPlanFactory.createPassivationAction();
	    	passivationAction.setComponent(str);
	    	passivationStep.getActions().add(passivationAction);
	    }

	    GettingStatusStep statusStep = reconfigurationPlanFactory.createGettingStatusStep();
	    plan.setGettingStatus(statusStep);
	    for (String str : getRemovedComponents()) {
	    	GettingOrdinaryStatusAction ordinaryStatusAction = reconfigurationPlanFactory.createGettingOrdinaryStatusAction();
	    	ordinaryStatusAction.setComponent(str);
	    	statusStep.getOrdinaryStatusAction().add(ordinaryStatusAction);
	    }
	    for (String starterComponent : getStarterComponents()) {
	    	GettingManagedStatusAction managedStatusAction = reconfigurationPlanFactory.createGettingManagedStatusAction();
	    	managedStatusAction.setComponent(starterComponent);
	    	statusStep.getManagedStatusAction().add(managedStatusAction);
	    }
	...
	//this procedure is to return a component group for changing
	...
	/*
	 * identify the frontier components from the removed components of the group
	 */
	ArrayList<String> getFrontierComponent() {
		ArrayList<String> frontierComponent = new ArrayList<String>();
		
		ArrayList<String> removedComponentsGroup = getRemovedComponents();
		ArrayList<Attachment> attachmentList = bm.getAttchmentList(bm.getSys());
		ArrayList<Binding> bindingList = bm.getBindingList(bm.getSys());		
		
		for (String component : removedComponentsGroup) {
			for (Attachment attachment : attachmentList) {
				if (component.equals(attachment.getComp()) && attachment.getRole().equals("callee")) {
					String connector = attachment.getCon();
					
					for (Attachment att : attachmentList) {
						if (!att.getComp().equals(component) && att.getCon()==connector && att.getRole()=="caller" ) {
							String srcComp = att.getComp();
							if (!removedComponentsGroup.contains(srcComp)) {
								
								frontierComponent.add(srcComp);
							}
						}
					}
				}
			...
			for (Binding binding : bindingList) {
				if (component.equals(binding.getCompDest())) {
					String srcComp = binding.getCompSrc();
					if (!removedComponentsGroup.contains(srcComp)) {
						frontierComponent.add(component);
					}
				}
			}
	...
	ArrayList<VSpecResolution> getStarterElements() { //in the variability model
		ArrayList<VSpecResolution> starterElements = new ArrayList<VSpecResolution>();
		...
		for (VSpecResolution removedElement : getRemovedElements()) {
			...
			for (BCLExpression bcl: dependsOn) {
				...
				VSpec src = ((VSpecRef)argument.get(0)).getVSpec();
				if (vspec.getName().equals(src.getName())) {
					starterElements.add(removedElement);
				}
			}
		}
		
		return starterElements;
	}
	
	ArrayList<String> getStarterComponents() {
		ArrayList<String> starterComponents= new ArrayList<String>();
		ArrayList<VSpecResolution> starterElements = getStarterElements();
		for (VSpecResolution starterElement : starterElements) {
			VSpec vspec = starterElement.getResolvedVSpec();
			
			ArrayList<VariationPoint> vps = vm.getVariationPoints();
			for (VariationPoint vp : vps) {
				if (vp.getBindingVSpec().getName().equals(vspec.getName())) {
					String starterComponent =  ((ObjectExistence)vp).getOptionalObject().getMOFRef();
					starterComponents.add(starterComponent);
				}
			}
		}
...
//	}
	...
	ArrayList<String> getCombinatedComponents() {
		...
		ArrayList<VSpecResolution> combinatedElements = getAddedElements();
		...
		for (VSpecResolution newElement : newElements) {
			...
			for (VariationPoint vp : vps) {
				if (vp.getBindingVSpec().getName().equals(vspec.getName())) {
					...
					}//statetransfersystem//ESBaseModel.xmi
				}
			}
		}
		return stp;
