...
import compression.control.CompressionOutInterceptor;
import compression.control.CompressionResponseInInterceptor;
import compression.control.Controller;

@Component(name = "Dispacher ")
public class Dispacher {
	...
	JaxWsProxyFactoryBean compression;
	CompressionService compressService;
	String compressionAddress = "udp://192.168.56.1:9010/rlecompression";
	public Compression() {
...
//		svrFactory.setDestinationFactory(destFac);
		...
		svrFactory.setAddress("udp://192.168.56.1:9001/compression"); 
		svrFactory.setServiceBean(sendingImpl);
		//svrFactory.getInInterceptors().add(new CompressionInInterceptor());
		svrFactory.getOutInterceptors().add(new CompressionOutInterceptor());
		
		
		server = svrFactory.create();
		server.start();
		
		//connect to RLEcompression 
		compression = new JaxWsProxyFactoryBean();
		compression.setServiceClass(CompressionService.class);
		compression.setAddress(compressionAddress);
		compression.getInInterceptors().add(new CompressionResponseInInterceptor());

		//compression.getOutInterceptors().add(new CompressionOutInterceptor());
		
		compressService = (CompressionService)compression.create();
...
//    	this.compressionAddress = address;
		if (oldAddress.equals(compressionAddress)) {
			compression.setAddress(newAddress); //"udp://192.168.56.22:9012/lzcompression";
			compressService = (CompressionService)compression.create();
		}
    }
	...
	class CompressionImpl implements CompressionService {
		public String compress(String source) {
			System.out.println("Compression receives: "+source);
			
			String compressedMag = compressService.compress(source);
			
			System.out.println("Compression receives the compressed message: "+compressedMag);
			
			return compressedMag;
		}
	}
